...
| `market_breadth_di` | —（衍生） | 由 `ods_stock_detail_di` 聚合，见 §3.4 |
...
| 6 | `market_breadth_di` | L4 | 市场风险偏好 | A | **待 ETL**（`ods_stock_detail_di` 聚合） |
...
| 10b | `ods_etf_basic_di` | L3/L6 | ETF↔跟踪指数映射 | A | **已配置**（`etf_basic` → full） |
...
| 逻辑表 | ODS | 状态 | 说明 |
|--------|-----|------|------|
| `market_breadth_di` | 衍生或 `ods_market_breadth_di` | 待 ETL | `ods_stock_detail_di` 聚合；或 `tdx_daily` |
| `etf_flow_di`（行业汇总） | — | 待 ETL | `ods_etf_share_size_di` + `ods_etf_basic_di` + `industry_etf_map` |
...
**依赖顺序**：`ods_index_member_all` → 财务/预期行业聚合；`ods_etf_basic_di` + `ods_etf_share_size_di` → 行业 ETF 资金流（另需 `industry_etf_map` 维表）。
...
行业汇总（逻辑 `etf_flow_di`）需 ETL：`ods_etf_share_size_di` + `ods_etf_basic_di` + `industry_etf_map`。
...
**MVP 建议**：ODS 已覆盖 S 级与大部分 A 级 raw 数据，可先实现 **资金 + 趋势** 两维（RS 基准已就绪）；情绪/扩散在 `ods_index_member_all` 映射规则明确后可快速补齐；景气/机构化需行业聚合 ETL；`market_breadth_di` 仍待衍生。
...
| 时间 | 任务 | 现状 |
|------|------|------|
| 16:30+ | `run_data_sync $trade_date` 跑 snapshot 类 ODS（`daily`/`sw_daily`/`moneyflow*`/`limit_list_d`/`index_daily`/`report_rc` 等） | 见 `调度执行流程.md` |
