...
| 表名 | 层级 | 本需求用途 | 优先级 |
|------|------|------------|--------|
| `stock_daily_di` | L1 | RS、N、U、龙头涨跌、A 杀 | S |
| `industry_index_di` | L2 | 行业收益、成交额、换手 | S |
| `industry_member_di` | L2 | 行业聚合、涨停统计分母 | S |
| `industry_fund_flow_di` | L3 | 辅助资金类校验 | S |
| `limit_up_di` | L4 | Z、J、B、LC | S |
| `etf_flow_di` | L3 | ETF 子因子 | A |
| `financial_indicator_di` | L5 | Earnings | A |
| `institution_forecast_di` | L6 | Forecast | A |

### 2.2 本需求新增表

| 表名 | 对应 Excel 层 | 说明 |
|------|---------------|------|
| `industry_etf_map` | — | 行业代码 ↔ ETF 代码、权重 |
...
| 子因子 | 计算逻辑 | 主要依赖 |
|--------|----------|----------|
| A | `industry_amount / market_amount` | industry_index_di |
| V | `MA5(amount) / MA20(amount)` | industry_index_di |
| ETF | `SUM(etf_net_inflow_5d) / SUM(etf_total_size)` 按 map 聚合 | etf_flow_di + industry_etf_map |
| H | `AVG(industry_turnover) / AVG(market_turnover)` | industry_index_di, stock_daily_di |
| RS | `ret_industry_20d / ret_benchmark_20d` | industry_index_di + 沪深300 |
| N | `COUNT(new_high_1y) / COUNT(members)` | stock_daily_di + industry_member_di |
| R | `drawdown_days / recovery_days` → 标准化取反 | industry_index_di |
| M | `SLOPE(MA30, 30)` | industry_index_di |
| Z | `limit_up_cnt / member_cnt` | limit_up_di + industry_member_di |
| J | `continue_limit / yesterday_limit` | limit_up_di |
| B | `1 - blast_cnt / touch_limit_cnt` | limit_up_di |
| U | `up_cnt / member_cnt` | stock_daily_di |
| LR | `leader_ret_20d - benchmark_ret_20d` | industry_leader_di |
| LC | `MAX(consecutive_limit)` in industry | limit_up_di |
| LP | 历史统计龙头次日收益 | 二期回测表 |
...
- [ ] `industry_index_di`、`industry_member_di`、`stock_daily_di`、`limit_up_di`
