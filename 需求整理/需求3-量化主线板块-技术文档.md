...
| `etf_flow_di` | 待建；源 **`etf_share_size`** + `industry_etf_map` | L3 | ETF 子因子 | A |
| `financial_indicator_di` | `ods_fina_indicator`（**已配置** `fina_indicator`） | L5 | Earnings | A |
...
| ETF 子因子 | `etf_share_size` + 维表 `industry_etf_map` | `etf_basic` 辅助 ETF↔指数映射 |
| Earnings | `fina_indicator` | 行业层按成分聚合 |
...
| ETF | `SUM(etf_net_inflow_5d) / SUM(etf_total_size)` 按 map 聚合 | `etf_flow_di`（源 `etf_share_size`）+ `industry_etf_map` |
...
- [x] `ods_fina_indicator`（`fina_indicator`，按股拉取，耗时长）
