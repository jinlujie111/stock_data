...
# -*- coding: utf-8 -*-
"""
临时脚本：回溯拉取 Tushare fina_indicator_vip → ods_fina_indicator。
按报告期 period 季末循环（全市场一季一次），默认区间 20200101 ~ 20260601。

用法（须先 source dw-utils/func.sh 或设置 MySQL / Tushare 环境变量）：
  python dw-tmp/tmp.py
  python dw-tmp/tmp.py --dry-run
  python dw-tmp/tmp.py --start 20200101 --end 20260601
  python dw-tmp/tmp.py --truncate --save-csv out/fina_indicator.csv
"""
...
def upsert_dataframe(df: pd.DataFrame, *, database: str, table: str) -> int:
    ...
    update_sql = ", ".join(f"`{c}`=VALUES(`{c}`)" for c in cols if c not in ("ts_code", "end_date", "ann_date"))
    sql = (
...
def truncate_table(*, database: str, table: str) -> None:
    engine = get_engine(database)
    with engine.begin() as conn:
        conn.execute(text(f"TRUNCATE TABLE `{table}`"))
    logger.info("已清空 %s.%s", database, table)
...
def fetch_periods(
    ...
    *,
    ...
    logger.info("共 %s 个报告期: %s ~ %s", len(periods), periods[0] if periods else "-", periods[-1] if periods else "-")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="临时回溯 fina_indicator_vip → ods_fina_indicator")
    parser.add_argument("--start", default=DEFAULT_START, help=f"起始日期 YYYYMMDD，默认 {DEFAULT_START}")
    parser.add_argument("--end", default=DEFAULT_END, help=f"截止日期 YYYYMMDD，默认 {DEFAULT_END}")
    parser.add_argument("--dry-run", action="store_true", help="只拉取统计，不写库")
    parser.add_argument("--truncate", action="store_true", help="写库前清空目标表")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if start > end:
        logger.error("start=%s 不能晚于 end=%s", args.start, args.end)
        return 1
    ...
    if args.truncate:
        truncate_table(database=TARGET_DATABASE, table=TARGET_TABLE)
        engine = get_engine(TARGET_DATABASE)
        out.to_sql(TARGET_TABLE, engine, if_exists="append", index=False, method="multi", chunksize=500)
        rows = len(out)
    else:
        rows = upsert_dataframe(out, database=TARGET_DATABASE, table=TARGET_TABLE)

    logger.info("写入 %s.%s 完成，%s 行", TARGET_DATABASE, TARGET_TABLE, rows)
    return 0
