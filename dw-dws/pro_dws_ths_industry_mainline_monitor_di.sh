#!/bin/bash
# =============================================================================
# target_table: dws_ths_industry_mainline_monitor_di
# 同花顺板块主线监控表，口径同 pro_dws_dc_industry_mainline_monitor_di.sh
#
# 用法:
#   bash dw-dws/pro_dws_ths_industry_mainline_monitor_di.sh 20260527
#   或: run_dws_ths_industry_mainline_monitor 20260527
# =============================================================================
...
mkdir -p "${LOG_PATH}"
exec 1>>"${LOG_PATH}/pro_dws_ths_industry_mainline_monitor_di_${n_date}.log"
exec 2>>"${LOG_PATH}/pro_dws_ths_industry_mainline_monitor_di_${n_date}.log"

echo "======== $(date '+%F %T') pro_dws_ths_industry_mainline_monitor_di ${n_date_s} ~ ${n_date_e} ========"

${data_mysql} -e "
CREATE TABLE IF NOT EXISTS dws_ths_industry_mainline_monitor_di (
    ...
    UNIQUE KEY uk_dws_ths_mainline_monitor (trade_date, industry_code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='同花顺板块主线监控表(DWS)';
"

load_ths_mainline_monitor() {
  local n_date="$1"
  local v_date
  ...
  score_cnt="$(${data_mysql} -N -e "
    SELECT COUNT(*) FROM dws_ths_industry_mainline_score_di WHERE trade_date = '${v_date}';
  ")"
  if [[ -z "${score_cnt}" || "${score_cnt}" -eq 0 ]]; then
    echo "WARN: skip ${v_date}, dws_ths_industry_mainline_score_di has no rows"
    return 1
  fi

  ${data_mysql} -e "
    DELETE FROM dws_ths_industry_mainline_monitor_di WHERE trade_date = '${v_date}';

    INSERT INTO dws_ths_industry_mainline_monitor_di (
        trade_date, content_type, industry_code, industry_name, rank_no, main_score,
    ...
    WITH enriched AS (
        ...
        FROM dws_ths_industry_mainline_score_di s
        LEFT JOIN dwm_ths_industry_fund_flow_di f
        ...
        LEFT JOIN dwm_ths_industry_diffusion_di df
    ...
    staged AS (
        SELECT e.*,
            CASE
                WHEN e.rs_5d > 0 AND e.lur_pctile >= 80 AND IFNULL(e.net_inflow_days, 0) >= 2 THEN '板块爆发'
                WHEN IFNULL(e.fund_accel, 0) > 0 AND IFNULL(e.net_inflow_days, 0) >= 1 THEN '资金试探'
                ELSE '观察'
    ...
    ranked AS (
        SELECT st.*,
            ROW_NUMBER() OVER (PARTITION BY st.trade_date, st.content_type ORDER BY st.main_score DESC) AS rank_no
        FROM staged st
    )
    SELECT trade_date, content_type, industry_code, industry_name, rank_no, main_score,
        total_score, total_score_ma3, total_score_ma5, total_score_ma10,
        mainline_level, mainline_stage, fund_cont_days, rs_5d, limit_up_cnt, profit_yoy,
        amount_ratio, limit_up_ratio, up_ratio,
  ...
  ${data_mysql} -e "
    ...
    FROM dws_ths_industry_mainline_monitor_di
    WHERE trade_date='${v_date}' AND content_type='行业' ORDER BY rank_no LIMIT 10;
  "
}

fail_cnt=0
for cur_date in $(get_continuous_date "${n_date_s}" "${n_date_e}"); do
  if ! load_ths_mainline_monitor "${cur_date}"; then
    fail_cnt=$((fail_cnt + 1))
  fi
done
