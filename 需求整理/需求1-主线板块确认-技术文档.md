...
| `stock_daily_di` | — | **未建表** |
| `industry_member_di`（成分） | — | **未建表**（需 `index_member` 等） |
| `limit_up_di` 等 | — | **未建表** |
...
| 7b | `industry_member_di`（成分股） | L2 | 涨停/上涨家数分母 | S | **待建设**（仅有分类，无成分） |
...
| 情绪类 | 涨停扩散率 | 涨停数 / 成分股总数 | `limit_up_di` + `ods_industry_member_di` | 否 |
| 情绪类 | 连板高度 | 行业内最高连板 | `limit_up_di` | 否 |
...
| 扩散类 | 上涨家数占比 | 上涨家数 / 成分总数 | `stock_daily_di` + 成分表 | 否 |
| 扩散类 | 20cm 扩散 | 创业板/科创板 20% 涨停数 | `limit_up_di` + `stock_daily_di` | 否 |

**MVP 建议**：在已同步 ODS 上先实现 **资金 + 趋势** 两维（及可算的成交额占比），热度/扩散/景气暂置中性分或 NULL，待 `limit_up_di`、成分股、财务表接入后补齐五维。
...
ODS 日快照任务（`moneyflow` / `moneyflow_ind_dc` / `sw_daily`）均依赖 `$trade_date`，须在 **T 日收盘后、Tushare 数据就绪后** 执行 `run_data_sync`；评分 ETL 应排在同步完成之后。
...
| proxy_source | source_table | target_table | sync_mode |
|--------------|--------------|--------------|-----------|
| tushare | `daily` | `ods_stock_daily_di` | snapshot |
| tushare | `limit_list` | `ods_limit_up_di` | snapshot |
| tushare | `index_member` | `ods_industry_member_di` | full / snapshot |
...
- [ ] `ods_stock_daily_di`（个股日线，对应逻辑 `stock_daily_di`）
- [ ] `ods_industry_member_di`（申万/指数成分）
- [ ] `ods_limit_up_di`（涨停清单）
