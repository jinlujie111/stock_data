(function () {
  ...
  async function loadDates() {
    ...
    if (!sid) return;
    const data = await apiGet(`/api/v1/rotation/strategies/${sid}/signal-dates`);
  ...
  async function loadSignals() {
    ...
    const data = await apiGet(`/api/v1/rotation/strategies/${sid}/signals${td}`);
  ...
  async function recordTrade(btn) {
    ...
    try {
      await apiPost("/api/v1/rotation/trades", {
