...
flowchart TB
    ...
    subgraph dm [主题层]
        ...
        SM[dwm_sector_dragon_summary_di]
        CFG[dwm_dc_sector_dragon_config]
    end
    subgraph app [应用层]
...
### 4.2 `dwm_dc_sector_dragon_config`（参数配置）
...
### 4.4 `dwm_sector_dragon_summary_di`（文字结论）
...
sequenceDiagram
    ...
    loop 每个 industry_code
        ...
        ETL->>DB: upsert dwm_sector_dragon_summary_di
    end
...
|-------|------|
| 评分表 | 股票 + 五列分数 + 排名 |
| 参数 | 权重、窗口与 `dwm_dc_sector_dragon_config` 对齐 |
...
- [ ] `dwm_dc_sector_dragon_config`、`dwm_sector_stock_dragon_score_di`、`dwm_sector_dragon_summary_di` DDL
...
- [ ] Excel 模板与导出
- [ ] 行业+概念周报推送（邮件/企微）
- [ ] `dwm_dc_sector_dragon_config` 管理页（按板块覆盖权重）
