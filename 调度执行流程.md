...
| 市场广度 DWM | `run_dwm_market_breadth 20260527` 或 `bash dw-dwm/pro_dwm_market_breadth_di.sh`（依赖当日 `ods_stock_detail_di` 已同步） |
| 行业-ETF 映射 DIM | `run_dim_industry_etf_map` 或 `bash dw_dim/pro_dim_industry_etf_map.sh`（依赖 `etf_basic`、`index_classify` 已同步） |
...
| `dw_dim/pro_dim_industry_etf_map.sh` | 行业-ETF 映射 DIM ETL |
