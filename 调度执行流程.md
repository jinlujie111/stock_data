...
任务定义在 `data_config.db_sync_task`（或预发下同库表）；**触发时间**由系统 **crontab** 或手工 `run_data_sync` 控制，不依赖外部调度平台。
...
```bash
cd /root/stock_data
source dw-utils/func.sh   # 生产；预发: DW_ENV=pre source dw-utils/func.sh
...
0 8 * * * cd /root/stock_data && DW_ENV=prod bash -lc 'source dw-utils/func.sh && run_data_sync $(date +\%Y\%m\%d)' >> /var/log/stock_data_sync.log 2>&1
```
...
| `Access denied` | 检查 `DW_ENV` 与 `env/prod.sh` / `env/pre.sh` 账号 |
...
| 文件 | 说明 |
|------|------|
| `dw-utils/func.sh` | 环境变量、`run_data_sync` |
| `dw-utils/env/prod.sh` / `pre.sh` | 生产 / 预发 MySQL |
