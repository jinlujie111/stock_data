(function () {
  ...
  async function loadStrategies() {
    ...
    elStrategy.innerHTML = items
      .map((s) => `<option value="${s.id}">${s.name}（${s.horizon === "long" ? "轮动" : "轮动"}）</option>`)
      .join("");
  }
  ...
  async function loadRuns() {
    const data = await apiGet("/api/v1/rotation/backtests");
    const items = data.items || [];
    if (!items.length) {
  ...
  async function loadDetail(runId) {
    const run = await apiGet(`/api/v1/rotation/backtests/${runId}`);
  ...
  async function runBacktest() {
    ...
    try {
      const res = await apiPost("/api/v1/rotation/backtests", {
        strategy_id: sid,
