...
> 版本：v1.2  
> 更新日期：2026-06-16  
> **实现口径（MVP）**：东财 `content_type=行业,概念`；**行业 Top10 + 概念 Top10 分别排名**；API `/api/v1/quant-mainline/top-groups`；页面 `/dc/quant-mainline`。
...
| `financial_indicator_di` | `ods_fina_indicator` + **`dwm_sw_industry_prosperity_di`** 等 | L5 | Earnings（`earnings_yoy`） | A |
| `institution_forecast_di` | **`dwm_*_industry_prosperity_di`**（源 `report_rc`） | L6 | Forecast（`upgrade_ratio`、`report_cnt_mom`） | A |
...
| Earnings | `fina_indicator_vip` → **`dwm_*_industry_prosperity_di`** | 行业层已 ETL；`run_data_sync --source-table fina_indicator_vip` |
| Forecast | `report_rc` → **景气 DWM** | `forecast_rev_pct`、`upgrade_ratio`、`report_cnt_*` |
| Policy | **景气 DWM `.policy_score`** | 占位 0；可扩展 `policy_score_di` 人工表 |
...
| `dwm_dc_mainline_config` | 05_参数配置 | 权重、阈值、版本、生效日 |
...
### 3.6 `dwm_dc_mainline_config`（05 参数配置）
...
| LP | 历史统计龙头次日收益 | 二期回测表 |
| Earnings | 行业净利润同比增速或一致预期 | `dwm_sw_industry_prosperity_di`.`earnings_yoy`（申万）；东财/同花顺用对应 `dwm_*_prosperity_di` |
| Policy | 政策热度 | `dwm_*_industry_prosperity_di`.`policy_score` |
| Forecast | 上调比例或研报环比 | `upgrade_ratio`、`report_cnt_mom`、`forecast_rev_pct` |
| RS / 趋势子因子 | 相对强度、均线多头等 | `dwm_*_industry_trend_strength_di`（与需求1 趋势维对齐） |
...
默认权重示例（需在 `dwm_dc_mainline_config` 落库，勿写死）：
...
建议调度（交易日）：**ODS + DWM 底座**见根目录 `XXL-JOB执行顺序.md` §6b（`run_data_sync` → 资金/趋势/景气 DWM，约 17:00–17:30）。量化评分层在 DWM 就绪后执行：

| 时间 | 任务 |
|------|------|
| 17:30+ | 景气 DWM 完成（`run_dwm_*_industry_prosperity`） |
| 19:00 | 因子原始值 + 标准化 + 主因子（读 DWM 表） |
...
- [x] `ods_fina_indicator`（`fina_indicator_vip`，`run_data_sync`）
- [x] 景气/趋势 DWM（`dwm_*_prosperity_di`、`dwm_*_trend_strength_di`，与需求1 共用）
- [ ] A、V、RS、Z、U 子因子 + F、T、E 主因子（P 可读景气 DWM；L 暂置中性分或省略降权）
- [ ] `industry_quant_mainline_di` + Top3 API

### Phase 2 — 完整 FTELP（+4 周）

- [ ] ETF 映射 + `etf_flow_di`；L 龙头规则 + LC/LR
- [ ] P：接入景气 DWM（Earnings/Forecast）；`policy_score` 人工/NLP 写入
- [ ] `industry_mainline_signal_di` 启动/退潮
- [ ] `dwm_dc_mainline_config` 热加载
...
| v1.2 | 2026-06-10 | P/T 子因子对齐景气、趋势 DWM；调度引用 `XXL-JOB执行顺序.md` §6b；Phase 1/2 进度更新 |
