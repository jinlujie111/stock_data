...
# DNS 解析失败时的代理 IP 兜底（Cloudflare，须配合 Host 头；见 test_tushare_network.sh）
TUSHARE_API_FALLBACK_IP="${TUSHARE_API_FALLBACK_IP:-}"
# 设为 1 则始终用域名、不替换为 IP
...
export TUSHARE_HTTP_URL TUSHARE_API_FALLBACK_IP TUSHARE_PROXY_USE_DOMAIN
