#!/bin/bash
# =============================================================================
# target_table: dwm_ths_industry_prosperity_di
# source_table: ods_ths_member_di, ods_ths_index_di, ods_ths_daily_di,
#               ods_fina_indicator, ods_report_rc_di
# 同花顺板块产业景气：成分股财务指标 + 卖方预测聚合（结构对齐东财 DWM）
#
# 指标口径：同 pro_dwm_dc_industry_prosperity_di.sh 注释块
# 板块范围 index_type I/N/R（行业/概念/地域）
#
# 用法（必须用 bash，不要用 sh）:
#   bash dw-dwm/pro_dwm_ths_industry_prosperity_di.sh              # 默认昨日
#   bash dw-dwm/pro_dwm_ths_industry_prosperity_di.sh 20260527
#   bash dw-dwm/pro_dwm_ths_industry_prosperity_di.sh 20260501 20260527
#   或: run_dwm_ths_industry_prosperity 20260527  （先 source dw-utils/func.sh）
# =============================================================================
...
mkdir -p "${LOG_PATH}"
exec 1>>"${LOG_PATH}/pro_dwm_ths_industry_prosperity_di_${n_date}.log"
exec 2>>"${LOG_PATH}/pro_dwm_ths_industry_prosperity_di_${n_date}.log"

echo "======== $(date '+%F %T') pro_dwm_ths_industry_prosperity_di ${n_date_s} ~ ${n_date_e} ========"

${data_mysql} -e "
CREATE TABLE IF NOT EXISTS dwm_ths_industry_prosperity_di (
    ...
    UNIQUE KEY uk_dwm_ths_industry_prosperity (trade_date, industry_code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='同花顺板块产业景气(DWM,成分股财务+卖方预测聚合)';
"

load_ths_industry_prosperity() {
  local n_date="$1"
  ...
  echo "DEBUG load_ths_industry_prosperity: n_date=${n_date} v_date=${v_date}"
  ...
  ${data_mysql} -e "
    DELETE FROM dwm_ths_industry_prosperity_di WHERE trade_date = '${v_date}';

    INSERT INTO dwm_ths_industry_prosperity_di (
        trade_date,
        content_type,
        industry_code,
        industry_name,
  ...
  ${data_mysql} -e "
    SELECT trade_date, content_type, industry_code, industry_name,
           earnings_yoy, forecast_rev_pct, upgrade_ratio, prosperity_rank
    FROM dwm_ths_industry_prosperity_di
    WHERE trade_date = '${v_date}'
...
run_dwm_by_trading_day "${n_date_s}" "${n_date_e}" load_ths_industry_prosperity || exit $?
