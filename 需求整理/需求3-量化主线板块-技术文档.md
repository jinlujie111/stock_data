...
> 版本：v1.2  
> 更新日期：2026-06-16  
> **实现口径（MVP）**：东财 `content_type=行业,概念`（可配置），表名前缀 `dws_dc_industry_quant_*`；Top3 在两类板块合并排序后取前 3；ETL 入口 `etl/quant_mainline/batch.py` / `run_dws_dc_industry_quant_mainline`；API `/api/v1/quant-mainline/*`；页面 `/dc/quant-mainline`。
...
建议调度（交易日）：**ODS + DWM 底座**见根目录 `调度执行流程.md` §6b（`run_data_sync` → 资金/趋势/景气 DWM，约 17:00–17:30）。量化评分层在 DWM 就绪后执行：
...
| v1.2 | 2026-06-10 | P/T 子因子对齐景气、趋势 DWM；调度引用 `调度执行流程.md` §6b；Phase 1/2 进度更新 |
