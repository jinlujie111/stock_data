...
| 8 | `ods_fina_indicator`（逻辑 `financial_indicator_di`） | L5 | 景气、业绩 | A | **已配置同步**（`fina_indicator_vip`） |
...
| — | tushare | `fina_indicator_vip` | `ods_fina_indicator` | snapshot | 全市场；`ann_date=$trade_date`（≥5000 积分） |
...
| `financial_indicator_di` | **`ods_fina_indicator`**（已配置） | **`fina_indicator_vip`**（全市场，`ann_date` 或 `period`） | `fina_indicator`（按股，≥2000）；`forecast`（预告） | 个股直出，**行业聚合** | `fina_indicator_vip` ≥5000 |
...
行业层：按 `ods_index_member_all` 聚合。来源：**`fina_indicator_vip`**（见 §3.4）。
...
| tushare | `fina_indicator_vip` | `ods_fina_indicator` | snapshot / full | **已配置**；full 按季 `period` |
