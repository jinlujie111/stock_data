...
| 财务指标历史回补 | `bash dw-sync/sync_ods_fina_indicator.sh`（默认 20250101 起；景气 DWM 前建议先跑通） |
...
| 需求1 主线榜 Web | `industry_fund_flow`：`/dc/mainline`；API `/api/v1/mainline/rank`（需 `stock_read_grants.sql`） |
| XXL-JOB 日/月/周批 | `dw-utils/xxl_daily_batch.sh` / `xxl_monthly_batch.sh` / `xxl_weekly_batch.sh` |
...
# 7) 维表（可周更；需求1 机构化阶段建议每周跑）
run_dim_industry_etf_map

# 8) 需求1 仅补跑主线（等价 xxl_mainline_batch.sh 单日）
# bash dw-utils/xxl_mainline_batch.sh "${TD}"
```

**日批一键**（XXL-JOB / crontab）：`bash dw-utils/xxl_daily_batch.sh [YYYYMMDD]`，详见 [XXL-JOB执行顺序.md](XXL-JOB执行顺序.md)。
...
# 或指定区间：bash dw-sync/sync_ods_fina_indicator.sh --start 20250101 --end 20260630
```


## 6c. 需求1 Web 与 API（`industry_fund_flow`）

日批步骤 ⑧（`run_dws_dc_industry_mainline_monitor`）完成后可读库展示。
...
|------|------|
| 页面 | `/dc/mainline` |
| API | `GET /api/v1/mainline/rank?trade_date=YYYYMMDD&top=20&ma_window=5` |
| 历史 | `GET /api/v1/mainline/history?industry_code=BKxxxx.DC` |
| 登录 Cookie | `iff_token` |

验收 SQL、API curl 示例见 [XXL-JOB执行顺序.md §九](XXL-JOB执行顺序.md)。
...
| `dw-sync/sync_ods_fina_indicator.sh` | `ods_fina_indicator` 按季回补（默认 202501 起） |
| `dw-dim/pro_dim_industry_etf_map.sh` | 行业-ETF 映射 DIM ETL |
| `dw-monitor/pro_ods_data_check.sh` | ODS 层数据缺失监控 |
| `dw-utils/xxl_daily_batch.sh` | XXL-JOB 日批一键 |
| `dw-utils/xxl_monthly_batch.sh` | 月批（stock_company / fina_mainbz） |
| `dw-utils/xxl_weekly_batch.sh` | 周批 ETF 映射 |
| `dw-utils/xxl_mainline_batch.sh` | 需求1 主线 DWM+DWS 补跑 |
| `industry_fund_flow/app/routes_mainline.py` | 需求1 主线榜 API / 页面 |
| `industry_fund_flow/sql/stock_read_grants.sql` | Web 读 `stock_data` 授权 |
| [XXL-JOB执行顺序.md](XXL-JOB执行顺序.md) | XXL-JOB 日批 / 月批 / 周批 / 验收 |
