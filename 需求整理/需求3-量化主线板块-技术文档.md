...
| Earnings | `fina_indicator_vip` → **`dwm_*_industry_prosperity_di`** | 行业层已 ETL；回补 `dw-tmp/sync_ods_fina_indicator.sh` |
...
- [x] `ods_fina_indicator`（`fina_indicator_vip`）；2025 起回补 `dw-tmp/sync_ods_fina_indicator.sh`
