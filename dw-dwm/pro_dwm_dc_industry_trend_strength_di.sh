#!/bin/bash
# =============================================================================
# target_table: dwm_dc_industry_trend_strength_di
# source_table: ods_dc_daily_di, ods_dc_index_di, ods_index_daily_di
# 东财板块趋势强度：RS、均线多头、60日新高、回撤修复
#
# 指标口径（与需求文档「趋势强度」对齐，基准=沪深300 000300.SH）：
...
#
# 要点：
#   - 回看窗口 120 自然日（≈85 交易日），用于 MA/RS/60日高点
#   - content_type 来自 ods_dc_index_di.idx_type（行业/概念/地域）
#
# 用法（必须用 bash，不要用 sh）:
#   bash dw-dwm/pro_dwm_dc_industry_trend_strength_di.sh              # 默认昨日
#   bash dw-dwm/pro_dwm_dc_industry_trend_strength_di.sh 20260527
#   bash dw-dwm/pro_dwm_dc_industry_trend_strength_di.sh 20260501 20260527
#   或: run_dwm_dc_industry_trend_strength 20260527  （先 source dw-utils/func.sh）
# =============================================================================
...
mkdir -p "${LOG_PATH}"
exec 1>>"${LOG_PATH}/pro_dwm_dc_industry_trend_strength_di_${n_date}.log"
exec 2>>"${LOG_PATH}/pro_dwm_dc_industry_trend_strength_di_${n_date}.log"

echo "======== $(date '+%F %T') pro_dwm_dc_industry_trend_strength_di ${n_date_s} ~ ${n_date_e} ========"
...
load_dc_industry_trend_strength() {
  local n_date="$1"
  local v_date v_date_120
  v_date="$(format_date "${n_date}")"
  v_date_120="$(date -d "${n_date} 120 day ago" +%Y-%m-%d)"
  echo "DEBUG load_dc_industry_trend_strength: n_date=${n_date} v_date=${v_date} v_date_120=${v_date_120}"
  ...
  ${data_mysql} -e "
    DELETE FROM dwm_dc_industry_trend_strength_di WHERE trade_date = '${v_date}';

    INSERT INTO dwm_dc_industry_trend_strength_di (
        trade_date,
        content_type,
        industry_code,
        industry_name,
...
run_dwm_by_trading_day "${n_date_s}" "${n_date_e}" load_dc_industry_trend_strength || exit $?
