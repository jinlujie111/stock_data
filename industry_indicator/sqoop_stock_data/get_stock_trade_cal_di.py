...
def delete_mysql_stock(table_name,start_date,end_date):
    ...
    print("执行先删除后插入操作，%s 表删除区间段数据： %s 到 %s 之前的数据" %(table_name,start_date,end_date) )
    sql="delete from %s where cal_date >= %s and cal_date <= %s ;" %(table_name,start_date,end_date)
    print(sql)
    connect.execute(sql)
    print('成功删除%s 表的 %s 到 %s 的数据' %(table_name,start_date,end_date) )
    connect.close()
...
def get_daily(start_date,end_date):
    ...
    pro = ts.pro_api()
    data_l = pro.trade_cal(exchange='', start_date=start_date, end_date=end_date)
    delete_mysql_stock('trade_cal', start_date,end_date)
    print("插入mysql中：%s - %s " %(start_date,end_date))
