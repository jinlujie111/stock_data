...
# 每天 08:30 同步后检查 ODS（有缺失则 exit 1，便于告警脚本捕获）
30 8 * * * cd /root/stock_data && bash -lc 'source dw-utils/func.sh && run_ods_data_check' >> /var/log/stock_ods_check.log 2>&1
```
...
**crontab 示例（交易日 T 日收盘后）**

```cron
# 08:00 ODS 全量 snapshot
0 8 * * * cd /root/stock_data && bash -lc 'source dw-utils/func.sh && run_data_sync $(date +\%Y\%m\%d)' >> /var/log/stock_data_sync.log 2>&1

# 17:00 起 DWM 链（行情/涨停/资金流/板块日线就绪后）
0 17 * * * cd /root/stock_data && bash -lc 'source dw-utils/func.sh && TD=$(date +\%Y\%m\%d) && run_dwm_market_breadth $TD && run_dwm_dc_industry_fund_flow $TD && run_dwm_ths_industry_fund_flow $TD && run_dwm_dc_industry_trend_strength $TD && run_dwm_ths_industry_trend_strength $TD' >> /var/log/stock_dwm.log 2>&1

# 17:30 产业景气（依赖 fina/report_rc；可与上条合并）
30 17 * * * cd /root/stock_data && bash -lc 'source dw-utils/func.sh && TD=$(date +\%Y\%m\%d) && run_dwm_dc_industry_prosperity $TD && run_dwm_ths_industry_prosperity $TD && run_dwm_sw_industry_prosperity $TD' >> /var/log/stock_dwm_prosperity.log 2>&1

# 每周一 06:00 刷新行业-ETF 维表
0 6 * * 1 cd /root/stock_data && bash -lc 'source dw-utils/func.sh && run_dim_industry_etf_map' >> /var/log/stock_dim.log 2>&1
```
